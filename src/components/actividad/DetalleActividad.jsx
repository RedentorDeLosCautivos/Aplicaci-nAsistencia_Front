...
import { TomarAsistencia } from "../asistencia/TomarAsistencia"; // 👈 Importamos el modal nuevo
...
export const DetalleActividad = () => {
  ...
  const [showTomarAsistencia, setShowTomarAsistencia] = useState(false); // 👈 nuevo estado
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] px-4 py-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* 👇 Nuevo botón para tomar asistencia manual */}
        <button
          onClick={() => setShowTomarAsistencia(true)}
          className="flex items-center justify-center gap-3 bg-[#142130] text-white py-4 rounded-2xl font-semibold text-base hover:bg-[#00020D] transition-all"
        >
          <UserCheck size={20} /> Tomar Asistencia
        </button>
      ...
      {showTomarAsistencia && (
        <TomarAsistencia
          actividadId={id}
          onClose={() => setShowTomarAsistencia(false)}
        />
      )}
    </div>
  );
